...
export const RuiRadio = ({id, defaultChecked, defaultValue, value, name, variant, color = "teal", disabled, checked, onChange, onClick }: RuiRadioProps) => {
    switch (variant) {
        ...
        case "bordered":
            return (
                <input type="radio" onChange={onChange} onClick={onClick} defaultValue={defaultValue} defaultChecked={defaultChecked} checked={checked} name={name} id={id} value={value} className={`appearance-none peer border-2 w-5 h-5 rounded-full checked:border-[6px] hover:border-opacity-80 ${disabled !== true ? `${radioColor[color][1]}` : "border-sylver-800 pointer-events-none"} duration-300`}></input>
            )
        case "ghost":
            return (
