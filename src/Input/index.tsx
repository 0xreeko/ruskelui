...
export const RuiInput = ({ id, disabled, isDark, label, placeholder = 'example@rhcp.com', pattern, max, min, variant, value, color = "amethyst", fullWidth = false, icon, iconPosition = 'right', ...props }: RuiInputProps) => {
    ...
    switch (variant) {
        ...
        case 'number':
            return (
                <input type="number" id={id} min={min} max={max} value={value} className="px-3 py-1 border border-gray-300 rounded-md outline-none appearance-none dark:border-gray-600 text-russian-600 dark:text-sylver-200 bg-white/20 dark:bg-black/20 backdrop-blur-sm" />
            )
        case 'time':
            return (
                <input type="time" id={id} data-isdark={`${isDark}`} value={value} className={`${styles.clocker}`} {...props} />
            )
        default:
            return (
                <div className={`relative group ${fullWidth ? "w-full" : "w-full max-w-xs"}`} >
