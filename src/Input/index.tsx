...
export const RuiInput = ({ id, disabled, isDark, label, placeholder = 'example@rhcp.com', pattern, max, min, variant, value, color = "amethyst", fullWidth = false, icon, iconPosition = 'right', ...props }: RuiInputProps) => {
    ...
    switch (variant) {
        ...
        case 'number':
            return (
                <div className={`relative ${fullWidth ? "w-full" : "w-full max-w-xs"}`} >
                    <input type="number" id={id} min={min} max={max} value={value} placeholder={placeholder} autoComplete={'off'} {...props} className={`peer w-full px-4 py-2 pr-10 border rounded-md outline-none ${inputColor[color][0]} sm:text-sm border-obsidian-300 dark:border-obsidian-800 text-neropside dark:text-onyx bg-white/20 dark:bg-black/20 backdrop-blur-sm`} />
                    <span className={`${inputColor[color][1]} absolute inset-y-0 place-content-center grid right-0 w-10 duration-200 pointer-events-none peer-placeholder-shown:text-obsidian-500 dark:peer-placeholder-shown:text-obsidian-400`}>
