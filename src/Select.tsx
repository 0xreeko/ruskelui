...
export const RuiSelect = ({label = "Choose an ISO20022 coin"}: Props) => {
    ...
    return (
        <div className="relative w-fit text-sylver-100">
            ...
            <ul tabIndex={0} className={clsx(`absolute origin-top text-[1rem] w-full z-10 mt-1.5 p-3 h-40 rounded-2xl overflow-scroll bg-russian-500`, `${hidden ? 'transform transition-all -scale-0 opacity-0 duration-300' : 'transform transition-all opacity-100 scale-100 duration-300'}`)} role="listbox">
                <SelectItem>
                    XRP
                </SelectItem>
                <SelectItem>
