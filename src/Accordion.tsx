    ...
    return (
        <div className={`${styles.wrapper} ${fullWidth === true ? `w-full` : `w-full max-w-xs`}`} tabIndex={0}>
            {data.map(({ label, content }, idx: number) => (
                <div>
                    <button className={`${styles.accordionItem} ${open !== idx ? `${hoverColor[color][0]}` : `${hoverColor[color][1]}`}`} onClick={() => toggle(idx)}>
                        <span className='text-left text-d-base'>{label}</span>
                        <span><svg xmlns="http://www.w3.org/2000/svg" className={`w-4 h-4 ${open !== idx ? `${styles.iconBase}` : `${styles.iconActive} ${hoverColor[color][1]}`}`} width={24} height={24} viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">
