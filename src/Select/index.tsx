...
export const SelectItem = ({ value, color = "amethyst", disabled }: RuiSelectChildProps) => {
    ...
    return (
        <li tabIndex={0} className={`flex justify-between duration-150 hover:text-obsidian-100 hover:cursor-pointer py-2 px-4 ${disabled !== true  ? selectColor[color] : "text-obsidian-800 pointer-events-none select-none"}`} onClick={() => handleChange?.(value)}>
            <span>{value}</span>
        ...
        <SelectContext.Provider value={{ label, currentValue, handleChange, selectedIcon }}>
            <div className="relative w-72 group">
                ...
                <button className={`peer flex w-full items-center justify-between border border-gray-300 dark:border-gray-800 rounded-lg bg-gray-200 dark:bg-russian-400/40 py-1 px-3`} onClick={() => onChange()}>
                    <span className="whitespace-nowrap text-russian-600 dark:text-obsidian-100">{currentValue !== "" ? currentValue : label}</span>
                    <div className="pl-3">
