...
import { RuiSelectChildProps, RuiSelectProps, selectItemColor, selectMarkerColor} from './Select'
...
export const SelectItem = ({ value, color = "amethyst", disabled }: RuiSelectChildProps) => {
    ...
    return (
        <li tabIndex={0} className={`flex justify-between duration-150 hover:text-onyx text-neropside dark:text-onyx dark:hover:text-neropside hover:cursor-pointer py-2 px-4 ${disabled !== true  ? selectItemColor[color] : "text-neropside/40 dark:text-onyx/40 pointer-events-none select-none"}`} onClick={() => handleChange?.(value)}>
            <span>{value}</span>
    ...
    return (
        <SelectContext.Provider value={{ label, currentValue, handleChange, selectedIcon }}>
            <div className="relative w-72 group">
                ...
                <button className={`peer flex w-full items-center justify-between border border-obsidian-300 dark:border-obsidian-800 rounded-lg bg-obsidian-200 dark:bg-obsidian-400/40 h-9 py-2 px-3`} onClick={() => onChange()}>
