...
export const RuiButton = ({ children, color = 'amethyst', size = "sm", disabled = false, variant = "filled", rightIcon, leftIcon, onClick }: RuiButtonProps) => {
  switch (variant) {
    ...
    case "icon":
      return (
        <button type="button" onClick={onClick} className="flex w-fit px-3 items-center rounded-[9px] bg-sylver-100 dark:bg-russian-800 dark:active:bg-gray-900/80 active:bg-gray-200 py-1.5 duration-150 disabled:pointer-events-none disabled:bg-gray-100 disabled:text-gray-300 dark:disabled:bg-gray-800 dark:disabled:text-gray-700 disabled:select-none" aria-readonly={disabled} disabled={disabled}>
         {children}
        </button>
      )
    default:
