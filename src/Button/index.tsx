...
export const RuiButton = ({ id, type, children, color = 'amethyst', size = "sm", disabled = false, variant = "filled", rightIcon, leftIcon, onClick }: RuiButtonProps) => {
  switch (variant) {
    case "ghost":
      return (
        <button id={id} type={type} onClick={onClick} className={`flex ${sizing[size]} w-fit items-center gap-3 rounded-[9px] border-[1.5px] bg-transparent ${ghostButtonStyles[color]} text-sm duration-150 disabled:pointer-events-none disabled:border-gray-300 disabled:text-gray-300 disabled:select-none`} disabled={disabled}>
          {leftIcon && leftIcon}
          <span>{children}</span>
          {rightIcon && rightIcon}
        </button>
    ...
    case "text":
      return (
        <button id={id} type={type} onClick={onClick} className={`flex ${sizing[size]} w-fit items-center ${textButtonStyles[color]} gap-3 rounded-[9px] border-[1.5px] border-transparent text-sm backdrop-blur-sm duration-150 disabled:pointer-events-none disabled:text-gray-300 disabled:select-none`} disabled={disabled}>
          {leftIcon && leftIcon}
          <span>{children}</span>
          {rightIcon && rightIcon}
        </button>
